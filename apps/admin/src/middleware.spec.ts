...
describe("session middleware", () => {
  ...
  it("does nothing for a visitor with no session", async () => {
  ...
  it("leaves a healthy token alone", async () => {
    await middleware(request({ [SESSION_COOKIES.access]: token(3000), [SESSION_COOKIES.refresh]: "r1" }));
  ...
  it("renews shortly before expiry, not after", async () => {
    await middleware(request({ [SESSION_COOKIES.access]: token(30), [SESSION_COOKIES.refresh]: "r1" }));

    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("renews when the access cookie has gone entirely", async () => {
    await middleware(request({ [SESSION_COOKIES.refresh]: "r1" }));
  ...
  it("renews rather than trusting a token it cannot parse", async () => {
    await middleware(request({ [SESSION_COOKIES.access]: "not-a-jwt", [SESSION_COOKIES.refresh]: "r1" }));
