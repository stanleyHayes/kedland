...
## September 2026 PDF update ledger

Source: `Kedland International School Website Update.pdf`. Owner: Codex. User authorized sequential implementation and direct publication to main. This update supersedes earlier blue-palette, named-Principal and starter-content decisions above.

| PDF item | Update                                                        | Status | Evidence                                                                                                                                                                                                                                                                                              |
| -------- | ------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1        | Permanently retire seeded media, gallery tiles and blog posts | DONE   | Removed seed writers, launch datasets, bundled media and public image/gallery fallbacks. Startup cleanup targets known retired identifiers and seed ownership; school-created posts/uploads remain. Repeat-run cleanup verified against MongoDB.                                                      |
| 2        | Replace blue theme throughout with supplied coral-peach       | DONE   | Sampled PDF swatch #F17B77. Shared warm tokens, peach structural bands, footer/wave, dashboard, shadows, focus colours and social metadata. Readable dark companion colours; native school crest retained.                                                                                            |
| 3        | About card: the Principal                                     | DONE   | Shared About route cards updated and rendered.                                                                                                                                                                                                                                                        |
| 4        | Our Story card: the Principal                                 | DONE   | Shared About route cards updated and rendered.                                                                                                                                                                                                                                                        |
| 5        | Mission/vision card: the Principal                            | DONE   | Shared About route cards updated and rendered.                                                                                                                                                                                                                                                        |
| 6        | Facilities card: the Principal                                | DONE   | Shared About route cards updated and rendered.                                                                                                                                                                                                                                                        |
| 7        | Site-wide Principal wording audit                             | DONE   | Code, navigation, metadata, default content and public CMS reads corrected. Idempotent saved-content migration preserves identifiers, URLs and unrelated edits.                                                                                                                                       |
| 8        | Primary wording across website                                | DONE   | Page copy, metadata, enquiry selection and authored news audited. Saved sections, post copy and FAQs migrate on API startup. Historical numbered enquiry records remain compatible.                                                                                                                   |
| 9        | Admin-editable Student Life images                            | DONE   | Opening image and each timeline card use CMS media references; gallery remains editable. Nested image picker supplies editable alt text and supports removal. Optional images do not block saving or adding cards. Verified admin form, database round trip, public API hydration and rendered image. |
| 10       | Footer agency legal name                                      | DONE   | XCreativs Technologies Limited.                                                                                                                                                                                                                                                                       |
| Release  | Checks, browser review, commit and push main                  | DONE   | Implementation `c12ca38` and this verification ledger published directly to main; pre-existing untracked PDFs and QA artifacts excluded.                                                                                                                                                              |

Verification (2026-09-12): full unit run passed, followed by focused regression checks after final changes (222 shared-contract, 232 UI, 572 API, 186 admin, 418 web and 12 testing-package tests). All 19 content API integration tests pass, including real MongoDB cleanup, saved-copy migration and Student Life image persistence. Strict type checks and lint pass; all production builds pass. Chromium reviewed all 14 public routes at 1440px and 390px: no old Principal/Primary wording in the desktop audit, no contrast violations, and no mobile horizontal overflow. The new palette was visually inspected on the values section and Student Life cards. Local previews were stopped before the final production rebuild to prevent stale ISR/chunk artifacts.

Follow-up palette correction (2026-09-12): the school identified unreadable dark-on-dark text in the Academics learning path and requested peach for the swept header panel, with deep peach replacing brown accents. Both panels now use peach surfaces; shared text, accent and shadow tokens use deeper shades of the supplied peach hue. The native crest is preserved. The earlier automated contrast scan missed the gradient-backed learning panel, so the new browser regression checks its actual painted background and text, as well as the header SVG colours.
